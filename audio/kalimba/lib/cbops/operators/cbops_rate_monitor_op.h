...
typedef struct rate_monitor_op{
    ...
    unsigned    *new_amount_ptr;      /* pointer to new amount - optinal input */
    unsigned    expected_acc_sum;     /* total samples we expect to have seen so far - sw mode only */
    int         acc_drift;            /* drift from expected_acc_sum - sw mode only */
