 ...
 *
 * This should be called each time the TTP-generating operator receives some data.
 * It updates internal state based on the given ToA and number of samples,
 * and provides the calculated time to play for the first sample. 
 */
extern void ttp_update_ttp_from_source_time(ttp_context *context, TIME toa, TIME source_time, ttp_status *status);
