...
/**
 * \brief  Function to get the rate_adjust operator if available for the overridden endpoint.
 *
 * \param  ep_hdl - Endpoint handle
 * \param  value - Pointer which contain the opdata for available standalone rate adjust operator.
 *
 * \return Success or failure.
 */
extern bool get_override_ep_rate_adjust_op(OVERRIDE_EP_HANDLE ep_hdl, uint32* value);
#endif /* INSTALL_DELEGATE_RATE_ADJUST_SUPPORT */
/**
 * \brief  Function to set whether hw warp to be applied directlt or accumulatively
 *
 * \param  ep_hdl - Endpoint handle
 * \param  value - Any non-zero value means to apply directly
 *
 * \return Success or failure.
 */
 ...
 *
 * \param  ep_hdl - Endpoint handle
 * \param  *value - Pointer which contain the value hw warp
 *
 * \return Success or failure.
 */
extern bool get_override_ep_current_hw_warp(OVERRIDE_EP_HANDLE ep_hdl, uint32* value);
