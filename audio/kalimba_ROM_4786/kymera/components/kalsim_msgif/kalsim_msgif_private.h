...
#define STOP_SERVICE_RESP_MSG_LEN          sizeof(service_stop_resp_struct)/sizeof(uint16)
...
typedef struct service_start_resp_struct{
    uint16    msg_id;
    uint16    serv_tag;
...
} service_start_resp_struct;

/** Service stop response structure */
typedef struct service_stop_resp_struct{
    uint16    msg_id;
    uint16    serv_tag;
    uint16    result;
} service_stop_resp_struct;
...
void stop_audio_data_service(service_identifier type, SUBSERV_TAG sstag);
