...
#define BUFFER_GET_WRITE_ADDR(dest_reg, cb_reg) dest_reg = M[cb_reg + $cbuffer.WRITE_ADDR_FIELD]; \
                                                dest_reg = dest_reg AND 0xFFFFFFFC
#define BUFFER_GET_READ_ADDR(dest_reg, cb_reg)  dest_reg = M[cb_reg + $cbuffer.READ_ADDR_FIELD]; \
                                                dest_reg = dest_reg AND 0xFFFFFFFC
#define BUFFER_MASK_WRITE_ADDR(reg)             reg = reg AND 0xFFFFFFFC
#define BUFFER_MASK_READ_ADDR(reg)              reg = reg AND 0xFFFFFFFC
...
// bit 15 :      0 - cbuffer has metadata associated
//               1 - cbuffer does NOT have metadata
.CONST    $cbuffer.METADATA_MASK             0x008000;
.CONST    $cbuffer.METADATA_MASK_SHIFT       15;
...
.CONST    $cbuffer.IN_PLACE_MASK_SHIFT       16;
