...
from ACAT.Core.exceptions import (
    AnalysisError, BundleMissingError, DebugInfoNoVariableError
)
...
class PoolInfo(Analysis.Analysis):
    ...
    def get_pool(self, pool):
        ...
        # Get the pool contents (could be slow on a live chip). This
        # relies on a slightly complicated (but safe) trick: the pool
        # variable (e.g. 'L_MemPool5') has address <initial_free_ptr>, so
        # 'get' that.
    ...
    def ret_get_watermarks(self):
        ...
        for pool in range(self.pool_count):
            ...
            blks_total = self.debuginfo.get_dm_const(
                self.pooldata[pool].get_member('numBlocksFree').address, 0
            )
    ...
    def _lookup_debuginfo(self):
        ...
        if Arch.kal_arch != 4:
            ...
            # For Crescendo, the initial pFirstFreeBlock is not stored in
            # dm_const So just look up the pool array by name

            try:
                self.debuginfo.get_var_strict("L_MemPool0")  # Check
                single_core = True
            except DebugInfoNoVariableError:
                single_core = False
            ...
            try:
                ...
                for pool in range(self.pool_count):
                    # For Crescendo, the initial pFirstFreeBlock is not
                    # stored in dm_const So just look up the pool array by
                    # name
                    if single_core:
    ...
    def _inspect_pool(self, pool):
        ...
        for blockid, blockvar in enumerate(poolvar.members):
            ...
            # word.
            if blockvar.value[0] == pool and blockvar.address not in free_block_pointers:
                # Allocated block.
                blockvar.free = False
                if self.pmalloc_debug_enabled:
        ...
        for addr in search_range:
            ...
            if (data - Arch.addr_per_word *
                ...
                in_range = self.pooldata.address <= addr < (self.pooldata.address + self.pooldata.size)
