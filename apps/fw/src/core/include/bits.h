...
#define bits_set(word_ptr, mask) ((void) (*(word_ptr) |= (mask)))

/** Sets mask-ed bits of word to 0. Other bits unchanged. */
#define bits_clear(word_ptr, mask) ((void) (*(word_ptr) &= ~(mask)))
 ...
 *
 * Use to generate nbit sub-field masks.
 *
...
/**
 * Can integer i be represented with the given number of bits?
 *
 * Validation of values destined for bit sub-fields.
 *
 * \pre nbits < bits in word!
 */
