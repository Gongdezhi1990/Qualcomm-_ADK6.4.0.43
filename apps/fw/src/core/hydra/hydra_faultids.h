...
/*    */
    ...
    /**
     * The CDA devices log buffer offset got corrupted during a reset.
    ...
    /**
     * The USB event descriptor is wrong.
     * The argument is event group in MSB and event index in LSB.
     */
    FAULT_HOSTIO_USB_EVENT_DESC_ERROR = 0x105b,

    /**
     * A timeout occurred whilst waiting for the number of pending TDs in a
     * USB2 TX endpoint to get to zero.
     */
    FAULT_HOSTIO_USB_ZERO_PENDING_TDS_TIMEOUT = 0x105c,

    /**
     * USB TX underrun occured. The host should be able to
     * recover, but that indicates some problems with fetching data
     * for USB device block. The argument is endpoint index.
     */
    FAULT_HOSTIO_USB_TX_UNDERRUN_ERROR = 0x105d,

    /**
     * A failure occurred whilst installing a key during configuration.
     */
    FAULT_CONMAN_INSTALL_KEY_FAILURE = 0x105e,

    /**
     * USB TX error while requesting or reading data from either
     * data or TD buffers. Transfer has been stopped and
     * the endpoint has been stalled. The argument is error code
     * in the high word and event index in the low word.
     */
    FAULT_HOSTIO_USB_TX_BUFFER_ERROR = 0x105f,

    /**
     * USB RX overrun occured. That means USB hardware can't store data
     * received over USB fast enough to keep up with the host.
     * The argument is endpoint index.
     */
    FAULT_HOSTIO_USB_RX_OVERRUN_ERROR = 0x1060,

    /**
     * NOTE: 0x17ff is the highest fault code in the Qualcomm range
