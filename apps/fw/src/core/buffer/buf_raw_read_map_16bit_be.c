/* Copyright (c) 2016 Qualcomm Technologies International, Ltd. */
/*    */
/**
 * \file
 * Contains the corresponding function.
 */

#include "buffer/buffer_private.h"

#if CHIP_HAS_MMU_BIG_ENDIAN_MAPPING /* This is not supported by the hardware */
/**
 * Return a 16-bit pointer to read data from a buffer, big-endian.
 */
const uint16 *buf_raw_read_map_16bit_be(const BUFFER *buf)
{
    mmu_read_port_open(buf->handle);
    return mmu_read_port_map_16bit_be(buf->outdex);
}
#endif
