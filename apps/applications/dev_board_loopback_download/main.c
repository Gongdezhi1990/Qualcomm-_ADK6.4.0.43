...
@file
    ...
    Simple application demonstrating simple stereo loopback using
    a downloadable capability.
...
/* Handle for the downloaded bundle (KCS), to be passed to
 * OperatorBundleUnload() when all instances of any capability are no
 * longer required, to free up audio subsystem memory */
static BundleID bID;

/* index used to refer to a specific file within the
 * filesystem */
static FILE_INDEX index=FILE_NONE;

/* The name of the bundle file containing two capabilities.
 * - Download Self Test
 * - Pass-through
 * Only Pass-through is used here, but this is an example of how
 * more than 1 capability can exist in a single KCS file.
 * The file referenced here is a result of building the
 * 'self_test_and_passthrough' project in this example's
 * workspace, and should be added to the 'customer_ro_filesystem'
 * project in the same workspace. */
static const char self_test_and_passthrough[] = "self_test_and_passthrough.dkcs";
...
void main(void)
{
     ...
     * audio interfaces, and download, instantiate, and configure
     * audio operators.
     */
 ...
 * demonstration application, downloads the capability bundle,
 * instantiates the passthrough operator from that, and
...
static void setup_loopback(void)
{
    ...
    /* Check whether the bundle file actually exists in
     * customer_ro_filesystem */
    index = FileFind(FILE_ROOT, self_test_and_passthrough, strlen(self_test_and_passthrough));

    if(index != FILE_NONE)
    {
        /* Download the operator bundle.
         * This call will fail if the bundle file is present but something
         * about its contents is invalid (for instance, if it was built for
         * a different device with a different audio ROM).
         */
        bID = PanicZero(OperatorBundleLoad(index, AUDIO_CORE_0));
        /* Now instantiate the "basic passthrough" capability from the
         * bundle as an operator, to copy the audio. */
