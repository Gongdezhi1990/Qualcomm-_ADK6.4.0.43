...
FILE NAME
    audio_input_analogue_broadcast.c
...
void audioInputAnalogueBroadcastCreate(Task input_task, Task output_task, audio_input_context_t *ctx)
{
    ...
    params.forwarding.pcm.left_source = ChainGetOutput(ctx->chain, path_pcm_output_left);
    params.forwarding.pcm.right_source = ChainGetOutput(ctx->chain, path_pcm_output_right);
