...
static const chain_config_t analogue_chain_config_mono_left =
    MAKE_CHAIN_CONFIG_WITH_PATHS(chain_id_audio_input_analogue, audio_ucid_input_analogue, op_config_mono, paths_mono_left);

static const chain_config_t analogue_chain_config_mono_right =
    MAKE_CHAIN_CONFIG_WITH_PATHS(chain_id_audio_input_analogue, audio_ucid_input_analogue, op_config_mono, paths_mono_right);

static const chain_config_t analogue_chain_config =
    MAKE_CHAIN_CONFIG_WITH_PATHS(chain_id_audio_input_analogue, audio_ucid_input_analogue, op_config, paths);
...
const chain_config_t* AudioInputAnalogueGetChainConfig(void)
{
    if (isMonoMode())
    {
	    return isLeftChannel() ? &analogue_chain_config_mono_left : &analogue_chain_config_mono_right;
    }
    else
    {
