...
DESCRIPTION
    Implementation of USB audio input plug-in.
*/
...
static audio_voice_context_t* usb_ctx = 0;


static const audio_voice_state_table_t usb_state_table =
{
    {
        [audio_voice_not_loaded] = {
                                        [audio_voice_connect_req] = {AudioInputUsbConnectAndFadeIn, audio_voice_connecting}
                                   },
...
static void setUpStateHandlers(void)
{
    ...
    usb_ctx = AudioVoiceCommonCreateContext();
    AudioVoiceCommonSetStateTable(&usb_state_table, usb_ctx);
}
...
void AudioVoiceUsbMessageHandler(Task task, MessageId id, Message message)
{
    ...
    AudioVoiceCommonMessageHandler(task, id, message, usb_ctx);

    /* Must only free context after transition to state audio_voice_not_loaded */
    if (usb_ctx && usb_ctx->state == audio_voice_not_loaded)
    {
        cleanContextData();
    }
}
